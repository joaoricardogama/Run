  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ borderCollapse: 'separate', borderSpacing: 0, width: '100%' }}>
        <thead>
          <tr>
            {rowLabel && <th style={{ width: 52, padding: '6px 4px', textAlign: 'left' }} />}
            {DAYS_LBL.map((d, i) => (
              <th
                key={i}
                className="text-xs font-bold uppercase tracking-widest"
                style={{ color: 'var(--text-muted)', padding: '6px 4px', textAlign: 'left', minWidth: 130 }}
              >
                {d}
              </th>
            ))}
...
export default function CoachCalendar() {
  ...
  const [athleteContent,  setAthleteContent]  = useState({})
  ...
  useEffect(() => {
    async function load() {
      setLoading(true)
      const { data } = await supabase.from('general_plans').select('*').eq('week_start', toISO(weekStart)).maybeSingle()
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase.from('athletes').select('id, name, group, email').order('name')
      setAthletes(data || [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      const { data } = await supabase.from('athlete_calendar').select('*')
        .eq('athlete_id', selectedAthlete.id).eq('week_start', toISO(weekStart)).maybeSingle()
  ...
  function setGenSessions(rowKey, day, sessions) {
    setContent(prev => ({ ...prev, [rowKey]: { ...(prev[rowKey] || {}), [day]: sessions } }))
  }
  function setAthSessions(day, sessions) {
    setAthleteContent(prev => ({ ...prev, [day]: sessions }))
  }

  function handleAdd(rowKey, day) { setModal({ rowKey, day, idx: null, session: null }) }
  function handleEdit(rowKey, day, idx) {
    const s = view === 'geral' ? content[rowKey]?.[day]?.[idx] : athleteContent[day]?.[idx]
    setModal({ rowKey, day, idx, session: s })
  }
  function handleRemove(rowKey, day, idx) {
    if (view === 'geral') {
      const s = [...(content[rowKey]?.[day] || [])]; s.splice(idx, 1); setGenSessions(rowKey, day, s)
    } else {
      const s = [...(athleteContent[day] || [])]; s.splice(idx, 1); setAthSessions(day, s)
    }
  }
  function handleModalSave(form) {
    const { rowKey, day, idx } = modal
    if (view === 'geral') {
      const s = [...(content[rowKey]?.[day] || [])]
      if (idx === null) s.push(form); else s[idx] = form
      setGenSessions(rowKey, day, s)
    } else {
      const s = [...(athleteContent[day] || [])]
      if (idx === null) s.push(form); else s[idx] = form
      setAthSessions(day, s)
    }
    setModal(null)
  }

  function handleDragStart(rowKey, day, idx, session) { setDragging({ rowKey, day, idx, session }) }
  ...
  function handleDrop(targetRowKey, targetDay) {
    ...
    if (view === 'geral') {
      const src = [...(content[srcRow]?.[srcDay] || [])]; src.splice(idx, 1)
      const tgt = [...(content[targetRowKey]?.[targetDay] || []), session]
      setContent(prev => ({
        ...prev,
        [srcRow]:       { ...(prev[srcRow]       || {}), [srcDay]:    src },
        [targetRowKey]: { ...(prev[targetRowKey] || {}), [targetDay]: tgt },
      }))
    } else {
      const src = [...(athleteContent[srcDay] || [])]; src.splice(idx, 1)
      const tgt = [...(athleteContent[targetDay] || []), session]
      setAthleteContent(prev => ({ ...prev, [srcDay]: src, [targetDay]: tgt }))
    }
    setDragging(null)
  }
  ...
  function handleAiGenerate() { setContent(generateWeekPlan(hasRace)); setShowAI(false) }

  const gridRows    = view === 'geral' ? [activeGroup] : ['athlete']
  const gridContent = view === 'geral' ? content : { athlete: athleteContent }
    ...
    <div className="flex flex-col h-full" style={{ background: 'var(--dark)', minHeight: '100vh' }}>
      ...
      <div className="flex items-center justify-between px-6 py-4 flex-shrink-0"
        ...
        <div className="flex items-center gap-1 p-1 rounded-xl" style={{ background: 'var(--surface2)' }}>
          {[{ key: 'geral', label: 'Geral', icon: CalendarDays }, { key: 'atleta', label: 'Atleta', icon: User }].map(({ key, label, icon: Icon }) => (
        ...
        <div className="flex items-center gap-3">
          ...
          <p className="text-sm font-bold" style={{ color: 'var(--text)' }}>{fmtWeek(weekStart)}</p>
          <button onClick={() => setWeekStart(w => addWeeks(w, 1))}
            className="w-8 h-8 rounded-lg flex items-center justify-center"
            style={{ background: 'var(--surface2)', color: 'var(--text-muted)' }}>
        ...
        <div className="flex items-center gap-2">
          {view === 'geral' && (
            <button onClick={() => setShowAI(true)}
              ...
              style={{ background: 'var(--surface2)', color: 'var(--text)', border: '1px solid var(--border)' }}>
              <Sparkles size={15} style={{ color: 'var(--orange)' }} />Gerar IA
            </button>
          )}
      ...
      <div className="flex items-center gap-2 px-6 py-3 flex-shrink-0"
        ...
        {view === 'geral' ? (
          <div className="flex gap-1.5">
            {GROUPS.map(g => {
              const clr = GROUP_COLOR[g]; const active = activeGroup === g
              return (
          ...
          <div className="flex items-center gap-3">
            ...
            <select value={selectedAthlete?.id || ''}
              ...
              {athletes.map(a => <option key={a.id} value={a.id}>{a.name} ({a.group})</option>)}
            </select>
            {selectedAthlete && <span className="text-xs" style={{ color: 'var(--text-muted)' }}>{selectedAthlete.email}</span>}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-auto p-6">
        {loading ? <LoadingSpinner /> : (
          <div className="rounded-2xl p-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            <CalendarGrid
              rows={gridRows} content={gridContent} dragging={dragging}
              onDrop={handleDrop} onAdd={handleAdd} onEdit={handleEdit} onRemove={handleRemove}
              onDragStart={handleDragStart} onDragEnd={handleDragEnd} rowLabel={false}
            />
          </div>
        )}
      </div>

      {modal && <SessionModal session={modal.session} onSave={handleModalSave} onClose={() => setModal(null)} />}

      {showAI && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          style={{ background: 'rgba(0,0,0,0.8)' }} onClick={() => setShowAI(false)}>
          <div className="w-full max-w-sm rounded-2xl p-6"
            ...
            <p className="text-sm font-bold mb-3" style={{ color: 'var(--text)' }}>Houve prova no fim de semana anterior?</p>
            <div className="flex gap-3 mb-6">
              {[{ v: true, l: '✓ Sim — com prova', d: '2ª feira CCL (recuperação)' }, { v: false, l: '✗ Não', d: '2ª feira CCN (carga normal)' }].map(({ v, l, d }) => (
                <button key={String(v)} onClick={() => setHasRace(v)}
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowAI(false)} className="flex-1 py-3 rounded-xl text-sm font-bold"
                style={{ background: 'var(--surface2)', color: 'var(--text)', border: '1px solid var(--border)' }}>Cancelar</button>
              <button onClick={handleAiGenerate} className="flex-1 py-3 rounded-xl text-sm font-bold"
                style={{ background: 'var(--orange)', color: 'white' }}>Gerar Plano</button>
            </div>
          </div>
        </div>
      )}
